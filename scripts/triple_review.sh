...
# Clean tempdir we cd into when running gemini so the CLI doesn't
# auto-load a stray .env file from the repo or the user's $HOME and
# silently override our env-i allowlist. (Gemini CLI is a Node app
# that runs dotenv at startup — env -u alone can't prevent that.)
GEMINI_TMPDIR="$(mktemp -d /tmp/review-gemini.XXXXXX)"
trap 'rm -rf "$DIFF_FILE" "$GEMINI_TMPDIR"' EXIT

# Pull in safe_env (env -i + explicit allowlist for subscription-CLI
# invocations). Pinned by tests/unit/test_safe_env_contract.py.
# shellcheck source=lib/safe_env.sh
. "$(dirname "$0")/lib/safe_env.sh"
...
trap 'rm -rf "$DIFF_FILE" "$PROMPT_FILE" "$GEMINI_TMPDIR"' EXIT
...
(
    set +e
    {
        ...
        echo
        # Allowlist via safe_env: only HOME/PATH/locale/XDG paths
        # cross over. Every OPENAI_*, CODEX_*, dotenv-loaded
        # override stays in the parent shell and is invisible here,
        # so subscription auth from ~/.codex is the only path to
        # billing.
        safe_env codex review --base "$BASE" --title "Triple-review: $HEAD_REF $HEAD_SHA" 2>&1
    } > "$OUT_CODEX"
) &
PID_CODEX=$!
...
(
    set +e
    {
        ...
        echo
        # Allowlist via safe_env. Additionally cd into an empty
        # tempdir before invoking gemini: the Node CLI runs dotenv
        # at startup and would auto-load any .env in the current
        # working directory (or its parents), silently restoring
...
(
    set +e
    {
        ...
        # (Anthropic Console subscription) stored in ~/.claude.
        # API-key mode is intentionally not supported — see
        # docs/triple_review.md "Why CLI subscription, not API".
        # Feed prompt + diff via stdin (not argv) to dodge ARG_MAX
        # on big diffs.
        #
        # safe_env's allowlist excludes every ANTHROPIC_API_KEY /
        # ANTHROPIC_AUTH_TOKEN / ANTHROPIC_BASE_URL variant, so a
        # stale API-key env var can't sneak in and silently bill the
        # API instead of the subscription session.
        {
            ...
            cat "$DIFF_FILE"
            echo '```'
        } | safe_env claude -p --add-dir "$REPO_ROOT" 2>&1
    } > "$OUT_CLAUDE"
) &
PID_CLAUDE=$!
