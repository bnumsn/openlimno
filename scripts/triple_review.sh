...
(
    set +e
    {
        ...
        echo
        # v0.1.0-final residual #1 (post-alpha.13 codex P2 + post-ship
        # eval) + RC review (Claude): strip the full OPENAI_*
        # environment family for the same reason claude has the
        # ANTHROPIC_* family stripped. ``OPENAI_BASE_URL`` would
        # silently re-route requests to a non-OpenAI endpoint;
        # ``OPENAI_ORG_ID`` could change billing.
        env -u OPENAI_API_KEY -u OPENAI_BASE_URL -u OPENAI_ORG_ID \
...
(
    set +e
    {
        ...
        echo
        # v0.1.0-final residual #1 + RC review: strip the full
        # GEMINI_* and GOOGLE_* env-var families. ``GOOGLE_API_KEY``
        # and ``GOOGLE_GENERATIVE_AI_API_KEY`` are honored by
        # gemini-cli alongside ``GEMINI_API_KEY``.
        env -u GEMINI_API_KEY -u GOOGLE_API_KEY -u GOOGLE_GENERATIVE_AI_API_KEY \
