...
| inSTREAM/NetLogo IBM | Implemented exchange bridge plus native research benchmark and optional NetLogo reference runner | Synthetic habitat export and population-summary CSV import tests; official inSTREAM 7.4 Example A/B depth/velocity matrices pass when `OPENLIMNO_RUN_INSTREAM_REAL=1`; native `ibm-benchmark-instream7` smoke runs against official A/B inputs with multi-day fish/redd state persistence and ordered-adjacent multi-reach movement; `ibm-run-instream7-netlogo-reference` copies/shortens official cases, forces daily BriefPop output, and was smoke-tested with NetLogo 7.0.2 Example A; NetLogo `BriefPopOut` parser/summarizer/comparator has unit and CLI coverage; short NetLogo 7.0.2 A/B summary fixtures pin the native-vs-NetLogo tolerance path; CLI export/inspect/import coverage | Add longer NetLogo reference-output artifacts and InSALMO-specific examples |
