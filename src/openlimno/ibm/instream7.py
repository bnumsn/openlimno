...
# 2026-05-26 R-IBM-GOD-OBJECT deeper split (ADR-0016 cleanup):
# brief-population parsing + native-vs-reference parity comparison
# extracted to instream7_compare.py. Re-exports preserve the
# existing public-API path used by cli.py + tests, AND make
# ``_column_lookup`` (small helper used by reach-static-cells
# readers ABOVE this point in the file) resolvable at call-time.
# Python name resolution is lazy inside function bodies, so the
# bottom-of-file import is sufficient.
from .instream7_compare import (  # noqa: E402, F401
    _column_lookup,
    compare_instream7_native_to_brief,
    read_instream7_brief_population,
    summarize_instream7_brief_population,
