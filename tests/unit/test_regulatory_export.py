...
def test_compute_sl712_columns_present() -> None:
    ...
    expected = {
        ...
        "monthly_avg_q_m3s",
        "min_eco_flow_m3s",
        "suitable_eco_flow_m3s",
        "multi_year_avg_pct",
        "min_eco_flow_p90_m3s",
    }
...
def test_compute_sl712_min_lt_suitable() -> None:
    ...
    res = cn_sl712.compute_sl712(
        Q, wua_q, "oncorhynchus_mykiss", "spawning", target_wua_pct=0.6, min_wua_pct=0.3
    )
    # min < suitable always
...
def test_compute_sl712_invalid_pct_thresholds() -> None:
    ...
    with pytest.raises(ValueError, match="min_wua_pct"):
        cn_sl712.compute_sl712(
            Q, wua_q, "oncorhynchus_mykiss", "spawning", target_wua_pct=0.3, min_wua_pct=0.6
        )
...
def test_compute_sl712_lemhi_real_data() -> None:
    ...
    wua_q = make_synthetic_wua_q().rename(
        columns={"wua_m2_oncorhynchus_mykiss_spawning": "wua_m2_oncorhynchus_mykiss_spawning"}
    )
    res = cn_sl712.compute_sl712(Q, wua_q, "oncorhynchus_mykiss", "spawning")
